...
for s_id in range(2,18):
    ...
    ecg = torch.tensor(ecg.copy(), dtype = torch.float32)
    null = torch.zeros_like(ecg)
    input_sig = torch.concat([ppg,null,ecg,null], dim = 1).unsqueeze(1)

    start = time.time()
    Model.eval()
    results = Model(input_sig.to(device), 100, '02', borrow_mode = 2, sample_steps=4, DDIM_flag=0)
    end = time.time()
    print((end-start)/60)
    r = results.detach().cpu().numpy()
    np.save(f'/mnt/vepfs/audio/lichang/HeartDiffuser/Heart_diffuser/WESAD_HR/WESAD_HR/results{s_id}.npy', r)
    print(s_id)
