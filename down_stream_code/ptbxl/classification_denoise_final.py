...
for index in range(4):
    sig = np.load(f'fifteenSNR{index}.npy')
...
train_mask = np.logical_or(train_label == 0, train_label == 2)
...
val_mask = np.logical_or(val_label == 0, val_label == 2)
...
test_mask = np.logical_or(test_label == 0, test_label == 2)
...
test_dataset_cleaned = custom_dataset(cleaned_signal[:,None,:][[test_mask]], test_label)
...
for iterations in range(10):
    ...
    def evaluate(model, data_loader, device):
        ...
        print("\nPer-class Metrics:")
        counter = 0
        for cls in ['Normal', 'HYP']:
            print(f"\n{cls}:")
            # print(f"Precision: {class_report[cls]['precision']:.3f}")
            if counter == 0:
                print(f"specificity: {class_report[cls]['recall']:.3f}")
            else:
                print(f"sensitivity: {class_report[cls]['recall']:.3f}")
