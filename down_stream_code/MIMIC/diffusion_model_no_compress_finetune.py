...
class CSDI_base(nn.Module):
    ...
    def finetune(
        ...
        mask_weight = self.create_combined_mask(observed_data[:,:,int(L/4):int(L/2)])
        noisy_data_m3 = torch.concat([noise, noisy_data_B, observed_data[:,:,int(L/2):int(3*L/4)], noise], dim=-1)               
        # Predict noise B
        predicted = self.diffmodel(noisy_data_m3, t, self.mask3.to(self.device), mode = 1, borrow_mode = 1) # Borrow mode doesn't matter in cross modality here
        predicted_x0 = (noisy_data_B - (1.0 - current_alpha) ** 0.5 * predicted)/(current_alpha ** 0.5)
        residual = noise - predicted # (B, 1, L)
        residual = residual*mask_weight
        loss1 = (residual ** 2).sum()/L
        loss2 = self.fft_loss(predicted_x0, observed_data[:,:,int(L/4):int(L/2)])
        loss3 = self.l1_loss(predicted_x0*mask_weight, observed_data[:,:,int(L/4):int(L/2)])
        loss = loss1 + loss2 + loss3
        
        return loss, loss1, loss2, loss3
        
    
    def create_combined_mask(self, bp_signal, peak_weight=3.0, valley_weight=2.0, base_weight=1.0):
        """
        GPU-friendly approach using gradients to identify peaks and valleys.
        """
        B, K, L = bp_signal.shape


        # Compute first and second derivatives
        first_deriv = torch.diff(bp_signal, dim=-1, prepend=bp_signal[..., :1])
        second_deriv = torch.diff(first_deriv, dim=-1, prepend=first_deriv[..., :1])

        # Smooth derivatives to reduce noise
        kernel = torch.ones(1, 1, 9, device=bp_signal.device) / 9
        first_deriv_smooth = F.conv1d(first_deriv, kernel, padding=4)
        second_deriv_smooth = F.conv1d(second_deriv, kernel, padding=4)
        # Create mask based on curvature and gradient

        # Peak regions: negative second derivative and small first derivative
        peak_mask1 = (second_deriv_smooth < -0.01) & (torch.abs(first_deriv_smooth) < 0.2)

        # Valley regions: positive second derivative and small first derivative  
        valley_mask1 = (second_deriv_smooth > 0.01) & (torch.abs(first_deriv_smooth) < 0.2)




        # Compute first and second derivatives
        average = torch.mean(bp_signal, dim=-1).unsqueeze(1)
        diff = (bp_signal - average)/torch.abs(average)
        # Create mask based on curvature and gradient
        mask = torch.ones_like(bp_signal)
        ...
        valley_mask = valley_mask1 | valley_mask2
        mask[peak_mask] = peak_weight
        mask[valley_mask] = valley_weight
        return mask   
...
#************************#
    ...
    def one_condition_imputation_x0(self, observed_data, n_samples, model_flag, borrow_mode):
        with torch.no_grad():
            B, K, L = observed_data.shape
            imputed_samples = torch.ones(B, n_samples, K, int(L/4)).to(self.device) # (B, N, 1, L)
            ...
            for i in range(n_samples):
                ...
                for t in range(self.num_steps - 1, -1, -1):
                    ...
                    if model_flag[0] == '0':
                        if model_flag[1] == '1':
                            predicted_x0 = (noisy_data[:,:,int(L/4):int(L/2)] - (1.0 - current_alpha) ** 0.5 * noise_predicted)/(current_alpha ** 0.5)
                            if t > 0:
                                # --- Calculate the Posterior Mean ---
                                # Coefficient for x0
                                posterior_mean_coef1 = (self.alpha[t-1]**0.5 * self.beta[t]) / (1 - self.alpha[t])
                                # Coefficient for xt
                                posterior_mean_coef2 = (self.alpha_hat[t]**0.5 * (1 - self.alpha[t-1])) / (1 - self.alpha[t])
                                # Full posterior mean
                                posterior_mean = posterior_mean_coef1 * predicted_x0 + posterior_mean_coef2 * noisy_data[:,:,int(L/4):int(L/2)]

                                # --- Add the Posterior Variance ---
                                # The standard deviation of the posterior
                                sigma = ((1.0 - self.alpha[t - 1]) / (1.0 - self.alpha[t]) * self.beta[t]) ** 0.5
                                
                                # The final sample for x_{t-1} is the mean + scaled noise
                                noisy_data[:,:,int(L/4):int(L/2)] = posterior_mean + sigma * noise
                            else:
                                # At the final step (t=0), the sample is just the mean (the final denoised output)
                                noisy_data[:,:,int(L/4):int(L/2)] = predicted_x0                 
                        elif model_flag[1] == '2':
                            noisy_data[:,:,int(L/2):int(3*L/4)] = coeff1 * (noisy_data[:,:,int(L/2):int(3*L/4)] - coeff2 * noise_predicted)
                            if t > 0:
                                noisy_data[:,:,int(L/2):int(3*L/4)] += sigma * noise
    ...
    def trainning(self, observed_data, sig_impute, sig_denoise, mask, task_dice, dirty_dice, condition_dice, train_threshold, stage, is_train=1, set_t=-1):
        ...
        loss, loss1, loss2, loss3 = self.finetune(observed_data, sig_impute, sig_denoise, mask, task_dice, dirty_dice, is_train=1, set_t=-1)
