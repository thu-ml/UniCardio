...
for i in range(0, 20):
    ...
    mean_results = torch.load(results_name).mean(dim=1)[:,0,:].detach().cpu().numpy()
    # for ii in range(mean_results.shape[0]):
    #     mean_results[ii] = -1 + 2*(mean_results[ii] - mean_results[ii].min())/(mean_results[ii].max() - mean_results[ii].min())
...
ECG_train, ECG_temp = train_test_split(
    ECG_generated, test_size=16000, random_state=42
)
